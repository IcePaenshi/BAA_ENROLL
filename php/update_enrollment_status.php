...
try {
    ...
    if ($status === 'rejected') {
        ...
        $firstName = baa_normalize_name_part((string) ($enrollment['first_name'] ?? ''));
        $middleName = baa_normalize_name_part((string) ($enrollment['middle_name'] ?? ''));
        $lastName = baa_normalize_name_part((string) ($enrollment['last_name'] ?? ''));
        $suffix = baa_normalize_name_part((string) ($enrollment['suffix'] ?? ''));
    ...
    // Notify student in-app if they have an account
    try {
        $getStudent = $pdo->prepare("SELECT id FROM users WHERE email = ? AND role = 'student' LIMIT 1");
        $getStudent->execute([$enrollment['email']]);
        $studentUserId = $getStudent->fetchColumn();

        if ($studentUserId) {
            $msg = "Your enrollment for level " . $enrollment['grade_level'] . " is now " . ucfirst($status) . ".";
            $notifStmt = $pdo->prepare("INSERT INTO notifications (user_id, type, message, link) VALUES (?, 'enrollment', ?, 'home')");
            $notifStmt->execute([$studentUserId, $msg]);
        }
    } catch (Exception $e) {
